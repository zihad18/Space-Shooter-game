...
void introScreen()
{
	...
	displayRasterText(-125, 225, 0.0,"Space Shooter Game");
   		glColor3f(1.0, 0.7, 0.8);

	  displayRasterText(-800, -100, 0.0," STUDENT NAME");
	  	glColor3f(1.0, 1.0, 1.0);
	displayRasterText(-800, -200, 0.0," Latifur Rahman Zihad");
	...
	displayRasterText(500, -200, 0.0,"Mostafijur Rahman Akhond");
		glColor3f(1.0, 0.0, 0.0);
	displayRasterText(-250, -400, 0.0,"Student id 180104");
        glColor3f(1.0, 1.0, 1.0);
...
void DrawLaser(int x, int y,float angle, bool dir[]) {
	//glPushMatrix();
	if(angle>=1.5708&&angle<=4.71239)
    {
        return;
    }
	int xend = -XMAX, yend = xend*tan(angle);
	if(dir[0])
		yend = YMAX;
	else if(dir[1])
		yend = -YMAX;
...
void checkLaserContact(int x, int y,float angle, bool dir[], int xp, int yp, bool player1) {
	if(angle>=1.5708&&angle<=4.71239)
    {
        return;
    }
...
void gameScreenDisplay()
{
	...
	if(alienLife1 > 0){
		...
		if(laser1) {
                float radian1=(angle1 * (3.14159265359 / 180));
			DrawLaser(xOne, yOne,radian1, laser1Dir);
			checkLaserContact(xOne, yOne,radian1, laser1Dir, -xTwo, yTwo, true);
		}
	}
	else {
		viewPage = GAMEOVER;
	...
	if(alienLife2 > 0) {
		...
		if(laser2) {
                 float radian2=(angle2 * (3.14159265359 / 180));
			DrawLaser(xTwo, yTwo,radian2, laser2Dir);
			checkLaserContact(xTwo, yTwo,radian2, laser2Dir, -xOne, yOne, false);
		}
		glPopMatrix();
	}
	else {
...
void keyOperations() {
	...
	if(viewPage == GAME) {
		...
		if(keyStates['c'] == true) {
			...
			if(keyStates['a'] == true) {glutPostRedisplay();
			angle2-=5.0;}
			if(keyStates['d'] == true) {
